...
*/
time_t get_seconds_from_ntp_server(uint8_t *buf, uint16_t idx)
{
...
int8_t SNTP_run(time_t* timeStamp)
{
	uint16_t RSR_len;
	uint32_t destip = 0;
	uint16_t destport;
	...
	case SOCK_UDP:
		...
		{
			do{
				ret += recvfrom(NTP_SOCKET, data_buf+ret,MAX_SNTP_BUF_SIZE, (uint8_t *)&destip, &destport);
			}while(getSn_RX_RSR(NTP_SOCKET) != 0);
			*timeStamp = get_seconds_from_ntp_server(data_buf,startindex);
			ntp_retry_cnt=0;
			close(NTP_SOCKET);
...
void calcdatetime(time_t seconds)
{
	uint8_t yf=0;
	time_t n=0,d=0,total_d=0,rz=0;
...
time_t changedatetime_to_seconds(void)
{
	time_t seconds=0;
